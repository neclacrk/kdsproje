...
<head>
	...
	function showGraph(){
		$.post("calisanannegrafikkontrol.php",
		function(data){
			console.log(data);
			var semt_adi=[];
			var calisan_anne_sayisi=[];
			for (var i in data){
				semt_adi.push(data[i].semt_adi);
				calisan_anne_sayisi.push(data[i].calisan_anne_sayisi);
			};
			var chartdata={
				labels:semt_adi,
				datasets:[
				{
				label:'Çalışan Anne Sayısı',
				data:calisan_anne_sayisi,
...
<div id="container">
	...
	<div id="ilk">
		...
		<div id="tablo"><p class="tabloyazisicalisan"><b>Çalışan Anne Sayısı Tablosu</b></p>
		
		<?php
$con = mysqli_connect('localhost','root','') or die ("Bağlantı başarısız");
mysqli_select_db($con,'kdsproje') or die ("veritabanı hatası");
mysqli_query($con,"SET NAMES UTF8");
$bilgi=mysqli_query($con,"select semtler.semt_adi, calisan_sayi.calisan_anne_sayisi from semtler,calisan_sayi,semt_calisan_sayisi WHERE semtler.semt_id=semt_calisan_sayisi.semt_id AND calisan_sayi.calisan_sayi_id=semt_calisan_sayisi.calisan_sayi_id");
echo '<table class="tablo" border="1" cellpadding="10" cellspacing="3">
	<tr>
		<td width="300" bgcolor="#006400"><b>SEMT ADI</b></td>
		<td width="300" bgcolor="#006400"><b>ÇALIŞAN ANNE SAYISI</b></td>

	</tr>';
while ($cek=mysqli_fetch_array($bilgi))	
{echo '	<tr>
		<td>'.$cek["semt_adi"].'</td>
		<td>'.$cek["calisan_anne_sayisi"].'</td>
