...
<head>
	...
	function showGraph(){
		$.post("hizmetguvengrafikkontrol.php",
		function(data){
			console.log(data);
			var semt_adi=[];
			var guven_orani=[];
			for (var i in data){
				...
				guven_orani.push(data[i].guven_orani);
			};
			var chartdata={
				labels:semt_adi,
				datasets:[
				{
				label:'Hizmet Kurumlarına Güven Oranı',
				data:guven_orani,
...
<div id="container">
	...
	<div id="ilk">
		...
		<div id="tablo"><p class="tabloyazisiguven"><b>Hizmet Kurumlarına Karşı Güven Oranı Tablosu</b></p>
		
		<?php
$con = mysqli_connect('localhost','root','') or die ("Bağlantı başarısız");
mysqli_select_db($con,'kdsproje') or die ("veritabanı hatası");
mysqli_query($con,"SET NAMES UTF8");
$bilgi=mysqli_query($con,"select semtler.semt_adi, hizmet_guven_orani.guven_orani from semtler,hizmet_guven_orani,semt_hizmet_guven WHERE semtler.semt_id=semt_hizmet_guven.semt_id AND hizmet_guven_orani.guven_orani_id=semt_hizmet_guven.guven_orani_id");
echo '<table class="tablo" border="1" cellpadding="10" cellspacing="3">
	<tr>
		<td width="300" bgcolor="#ee0000"><b>SEMT ADI</b></td>
		<td width="300" bgcolor="#ee0000"><b>HİZMET KURUMLARINA GÜVEN ORANI</b></td>

	</tr>';
while ($cek=mysqli_fetch_array($bilgi))	
{echo '	<tr>
		<td>'.$cek["semt_adi"].'</td>
		<td>'.$cek["guven_orani"].'</td>
