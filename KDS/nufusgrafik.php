		...
		function showGraph(){
			...
			function(data){
				...
				for (var i in data){
					...
					toplam_nufus.push(data[i].toplam_nufus);
				};
				var chartdata={
					labels:semt_adi,
					datasets:[
...
<div id="container">
	...
	<div id="ilk">
		...
		<div id="tablo"><p class="tabloyazisi"><b>Toplam Nüfus Tablosu</p></b>
		
		<?php
$con = mysqli_connect('localhost','root','') or die ("Bağlantı başarısız");
mysqli_select_db($con,'kdsproje') or die ("veritabanı hatası");
mysqli_query($con,"SET NAMES UTF8");
$bilgi=mysqli_query($con,"select semtler.semt_adi,nufus.toplam_nufus from semtler,nufus,semt_nufus WHERE semtler.semt_id=semt_nufus.semt_id AND nufus.nufus_id=semt_nufus.nufus_id");
echo '<table class="tablo" border="1" cellpadding="10" cellspacing="3">
	<tr>
		<td width="300" bgcolor="#ffa500"><b>SEMT ADI</b></td>
		<td width="300" bgcolor="#ffa500"><b>TOPLAM NÜFUS</b></td>

	</tr>';
while ($cek=mysqli_fetch_array($bilgi))	
{echo '	<tr>
		<td>'.$cek["semt_adi"].'</td>
		<td>'.$cek["toplam_nufus"].'</td>
