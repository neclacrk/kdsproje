...
<head>
	...
	function showGraph(){
		$.post("gelirgrafikkontrol.php",
		function(data){
			console.log(data);
			var semt_adi=[];
			var ortalama_aylik_gelir=[];
			for (var i in data){
				semt_adi.push(data[i].semt_adi);
				ortalama_aylik_gelir.push(data[i].ortalama_aylik_gelir);
			};
			var chartdata={
				labels:semt_adi,
				datasets:[
				{
				label:'Ortalama Aylık Gelir',
				data:ortalama_aylik_gelir,
...
<div id="container">
	...
	<div id="ilk">
		...
		<div id="tablo"><p class="tabloyazisigelir"><b>Ortalama Aylık Gelir Tablosu</b></p>
		
		<?php
$con = mysqli_connect('localhost','root','') or die ("Bağlantı başarısız");
mysqli_select_db($con,'kdsproje') or die ("veritabanı hatası");
mysqli_query($con,"SET NAMES UTF8");
$bilgi=mysqli_query($con,"select semtler.semt_adi,gelir.ortalama_aylik_gelir from semtler,gelir,semt_gelir WHERE semtler.semt_id=semt_gelir.semt_id AND gelir.gelir_id=semt_gelir.gelir_id");
echo '<table class="tablo" border="1" cellpadding="10" cellspacing="3">
	<tr>
		<td width="300" bgcolor="#2b547f"><b>SEMT ADI</b></td>
		<td width="300" bgcolor="#2b547f"><b>ORTALAMA AYLIK GELİR</b></td>
	</tr>';
while ($cek=mysqli_fetch_array($bilgi))	
{echo '	<tr>
		<td>'.$cek["semt_adi"].'</td>
		<td>'.$cek["ortalama_aylik_gelir"].'</td>
